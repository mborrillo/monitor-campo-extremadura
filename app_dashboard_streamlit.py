...
PAGES = {
    ...
    "Mapa de Operaciones": {"icon": "🗺️", "badge": "🗺️ Geolocalización",   "sub": "Red de estaciones de Extremadura"},
    "Monitor de Mercados": {"icon": "📊", "badge": "📊 Análisis mercado",   "sub": "Precios agrícolas y comparativa"},
...
.sticky-hero .page-hero {
    margin-bottom: 0 !important;
    border-radius: 0 0 var(--radius) var(--radius) !important;
    box-shadow: 0 4px 20px rgba(13,43,26,0.18) !important;
}
/* ── page-hero base (si no es sticky) ── */
.page-hero {
...
.page-hero h1 { color: white !important; font-size: 1.55rem; font-weight: 800; margin: 0 0 4px; line-height: 1.2; }
.page-hero p  { color: rgba(255,255,255,0.7); margin: 0; font-size: 0.85rem; }
.hero-badge {
...
/* ── Sidebar toggle nativo: reposicionarlo en la navbar ── */
...
/* Sidebar cerrado: mover el botón al borde izquierdo */
...
def render_dashboard():
    ...
    <div class="sticky-hero">
      <div class="page-hero">
        ...
        <p>Estado actual del campo extremeño — datos en tiempo real</p>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_g:
        section_header("📈", "Temperatura actual por estación", "v_mapa_operaciones — valores en tiempo real")
        if not df_mapa.empty and "temp_actual" in df_mapa.columns and "estacion" in df_mapa.columns:
            df_bar = df_mapa.sort_values("temp_actual", ascending=True).copy()
            # Color por rango de temperatura
            def temp_color(t):
                t = float(t or 0)
                if t >= 35: return "#ef4444"
                if t >= 28: return "#f59e0b"
                if t >= 20: return "#27a05e"
                return "#3b82f6"
            bar_colors = [temp_color(t) for t in df_bar["temp_actual"]]
            fig = go.Figure(go.Bar(
                x=df_bar["temp_actual"],
                y=df_bar["estacion"],
                orientation="h",
                marker=dict(color=bar_colors, opacity=0.9),
                text=[f"{v:.1f}°C" for v in df_bar["temp_actual"]],
                textposition="outside",
                textfont=dict(size=12, family="Plus Jakarta Sans", color="#0d2b1a"),
                hovertemplate="<b>%{y}</b><br>🌡️ %{x:.1f}°C<extra></extra>",
            ))
            # Línea de referencia media
            media = df_bar["temp_actual"].mean()
            fig.add_vline(x=media, line_dash="dot", line_color="#7aa98e", line_width=2,
                         annotation_text=f" Media: {media:.1f}°C",
                         annotation_font=dict(size=10, color="#7aa98e"))
            layout_bar = {**CHART_LAYOUT}
            layout_bar["xaxis"] = dict(showgrid=True, gridcolor="#e8f5ee", color="#7aa98e", title="°C")
            layout_bar["yaxis"] = dict(showgrid=False, color="#0d2b1a", automargin=True)
            layout_bar["margin"] = dict(l=0, r=60, t=0, b=0)
            fig.update_layout(height=max(200, len(df_bar)*42), showlegend=False, **layout_bar)
            st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
            # Leyenda colores
            st.markdown("""
            <div style="display:flex;gap:16px;flex-wrap:wrap;margin-top:4px;">
                <span style="font-size:0.75rem;color:#4a7c5f;">
                    <span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:#3b82f6;margin-right:4px;"></span>&lt; 20°C
                </span>
                <span style="font-size:0.75rem;color:#4a7c5f;">
                    <span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:#27a05e;margin-right:4px;"></span>20–28°C Normal
                </span>
                <span style="font-size:0.75rem;color:#4a7c5f;">
                    <span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:#f59e0b;margin-right:4px;"></span>28–35°C Calor
                </span>
                <span style="font-size:0.75rem;color:#4a7c5f;">
                    <span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:#ef4444;margin-right:4px;"></span>≥ 35°C Extremo
                </span>
            </div>
            """, unsafe_allow_html=True)
        else:
            st.info("Sin datos de temperatura por estación")
...
def render_mapa():
    ...
    <div class="sticky-hero">
      <div class="page-hero">
        ...
        <p>Red de monitorización agrometeorológica de Extremadura</p>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
def render_mercados():
    ...
    <div class="sticky-hero">
      <div class="page-hero">
        <span class="hero-badge">📊 Análisis de mercado</span>
        <h1>Monitor de Mercados</h1>
        <p>Precios agrícolas locales vs internacionales — lonja de Extremadura</p>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
def render_alertas():
    ...
    <div class="sticky-hero">
      <div class="page-hero">
        ...
        <p>Clima extremo y energía — actualizados automáticamente</p>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
def render_configuracion():
    ...
    <div class="sticky-hero">
      <div class="page-hero">
        ...
        <p>Secrets de Supabase, AEMET y diagnóstico de conexiones</p>
      </div>
    </div>
    """, unsafe_allow_html=True)
    col1, col2 = st.columns(2)
